            ...
            for (cgltf_size nodeIndex = 0; nodeIndex < data->nodes_count; nodeIndex++)
            {
                ...
                if (node.camera) {
                    if (!strcmp(node.name, cameraName))
                    {
                        ...
                        p->glhPerspectivef2Rad(node.camera->data.perspective.yfov, node.camera->data.perspective.aspect_ratio/*320.f / 200.f*/, gltfFrame.znear, gltfFrame.zfar, true, true);
